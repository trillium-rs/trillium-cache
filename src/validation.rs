...
impl CachePolicy {
    /// Decide what to do with a new request given this stored response.
    ///
    /// Returns:
    /// - [`BeforeRequest::Fresh`] when the cached response satisfies the request directly. Pair the
    ///   returned [`CachedResponse`] head with the body bytes from storage.
    /// - [`BeforeRequest::Stale`] otherwise. Send the request to the origin using the supplied
    ///   `request_headers` (which may carry conditional validators for a 304 round-trip). The
    ///   `matches` flag is `true` when this entry's vary signature matches the request; `false`
    ///   means it doesn't — consider another stored candidate, or send unconditionally.
    ...
    /// The response head to return on a cache hit. Updates `Age`,
    /// strips hop-by-hop headers, preserves the origin's `Date`. Pair
    /// with the body bytes from storage to construct the served
    /// response.
    ///
    /// `Date` is deliberately preserved as the origin provided it
    /// rather than rewritten to the current time — recipients combine
    /// `Date` and `Age` to reason about effective freshness, and
    /// overwriting `Date` would make that math misleading.
    ...
    /// Integrate the origin's response to a revalidation request.
    ///
    ...
    ///   stored entry. Reuse the cached body and serve the returned `CachedResponse` head; replace
    ///   the stored entry with the included `CachePolicy` (which carries the merged stored+304
    ///   headers and a refreshed `response_time`).
    /// - [`AfterResponse::Modified`] otherwise. Read the body off the origin response and serve it;
    ///   check [`CachePolicy::is_storable`] before persisting the new entry.
...
pub(crate) enum BeforeRequest {
    ...
    // The cached response is fresh AND the request's conditional headers
    // indicate the client already has the current representation. Serve the
    // returned head with status 304 and an empty body.
    ...
    Stale {
        ...
        // policy could derive).
        request_headers: Headers,
        // True when the stored entry actually matches this request's vary
        // signature. False means this entry doesn't match; consider another
        // stored candidate or send unconditionally.
        matches: bool,
    },
}
...
pub(crate) enum AfterResponse {
    // Origin returned 304 Not Modified with validators matching the stored
    // entry. Reuse the cached body, serve the returned `CachedResponse` head,
    // and replace the stored policy with the included new one (merged headers
    // + refreshed `response_time`).
    NotModified(CachePolicy, CachedResponse),
    // Origin returned a fresh response. Pass the origin response through
    // unchanged; treat as a fresh miss against this key (drop the stale
    // entry, check `CachePolicy::is_storable`, and store if storable).
    Modified,
}
